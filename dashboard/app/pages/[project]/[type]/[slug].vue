...
// Page for specific Artifact Viewer
definePageMeta({
  layout: 'dashboard'
})
...
const deleteArtifact = async () => {
  ...
  try {
    await $fetch(`/api/projects/${project.value}/artifacts/${type.value}/${slug.value}`, {
      method: 'DELETE'
    })
    isDeleteModalOpen.value = false
    // Redirect back to project home or brain home
    router.push('/')
  } catch (err) {
...
const updateStatus = async (newStatus: string) => {
  ...
  try {
    await $fetch(`/api/projects/${project.value}/artifacts/${type.value}/${slug.value}/status`, {
      method: 'PATCH',
...
const { data: artifact, status, refresh } = await useFetch(() => 
  `/api/projects/${project.value}/artifacts/${type.value}/${slug.value}`,
  { watch: [project, type, slug] }
)

// Computed helpers for layout
const metadata = computed(() => {
  if (!artifact.value?.frontmatter) return {}
  const obj: Record<string, string> = {}
  artifact.value.frontmatter.split('\n').forEach(line => {
...
const mainContentRaw = computed(() => {
  ...
  return artifact.value.content.replace(/^[\s\S]*?---[\s\S]*?---\s*/, '').trim()
})
...
const mainContent = computed(() => {
  ...
  // Strip the first paragraph if it's identical to our extracted description
...
const canComment = computed(() => type.value === 'plan' || type.value === 'plans')

// Analytics / Meta
useHead({
  title: () => artifact.value?.slug ? `${artifact.value.slug} - BYOBrain` : 'Loading...'
})
</script>

<template>
  <div class="glass p-8 rounded-2xl min-h-full flex flex-col">
    <div v-if="artifact" class="flex flex-col">
      <header class="mb-10 pb-6 border-b border-white/5 flex justify-between items-start">
        <div>
          ...
          <div class="flex flex-wrap gap-4 items-center text-[9px] font-black uppercase tracking-[0.2em] text-slate-700 mb-6">
            <div class="flex items-center gap-2">
              <span>PROJECT</span>
              <span class="text-indigo-400/60">{{ project }}</span>
            </div>
            <div class="flex items-center gap-2">
              <span>TYPE</span>
        ...
        <div class="flex gap-4 items-center">
          <button 
            v-if="canComment"
            @click="isCommentPanelOpen = !isCommentPanelOpen"
            class="p-2 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 border border-white/5"
            :class="isCommentPanelOpen ? 'bg-indigo-600 text-white border-indigo-500 shadow-lg shadow-indigo-600/20' : 'glass text-slate-400 hover:text-white'"
          >
          ...
          <div class="glass max-w-sm w-full p-8 rounded-3xl border border-white/10 shadow-2xl overflow-hidden relative">
            ...
            <div class="relative z-10">
              <div class="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 class="text-xl font-bold text-white">Delete Artifact?</h3>
              ...
              <div class="bg-white/5 rounded-2xl p-4 mb-8 border border-white/5">
                ...
                <p class="text-sm font-mono text-slate-300 break-all">{{ project }}/{{ type }}/{{ slug }}.md</p>
              </div>
    ...
    <div v-else class="h-full flex flex-col items-center justify-center text-center">
      ...
      <h2 class="text-2xl font-bold mb-2">Artifact Not Found</h2>
      <p class="text-slate-500 max-w-sm">The requested artifact could not be located in your brain.</p>
    </div>
  </div>
</template>
