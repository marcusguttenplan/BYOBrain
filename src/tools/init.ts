        ...
        async ({ project, repo_path }) => {
            ...
            const brainSection = (agentName: string) =>
                [
                    ...
                    "2. **Plans** — Plans are **full-fidelity** copies of the agreed Implementation Plan (no size limit). Always include `revision_comment` when updating to maintain the running revision log.",
                    "3. **Tasks** — Write to `tasks/{slug}.md` as you make progress.",
                    "4. **Issues** — New blockers → `issues/{slug}.md`. Resolved → set `status: resolved`.",
                    "5. **Commands** — If a complex terminal command succeeds after trial-and-error, call `save_command` to harvest it in the `{project}/commands` catalog.",
                    "6. **Knowledge** — Create KIs **iteratively** as you process code. Don't wait for a pattern to be 'solidified' — capture discoveries, API signatures, gotchas, and architectural insights as you encounter them.",
                    "",
